...
RegisterServerEvent('loaf_character:getChars')
AddEventHandler('loaf_character:getChars', function()
    local src = source
    SetIdentifierToChar(GetPlayerIdentifiers(src)[1], GetLastCharacter(src))
    Wait(250)
    TriggerClientEvent('loaf_character:loadCharacters', src, GetPlayerCharacters(src), false)
end)
...
AddEventHandler('kashactersS:CharacterChosen', function(charid, ischar)
    ...
    SetCharToIdentifier(GetPlayerIdentifiers(src)[1], charid)
    if ischar then
        spawn = GetSpawnPos(src)
    else
        TriggerClientEvent('skinchanger:loadDefaultModel', src, true, cb)
        spawn = {x= -214.35, y = -1011.36, z = 28.39} -- DEFAULT SPAWN POSITION
    end
    Wait(100)
...
AddEventHandler('kashactersS:DeleteCharacter', function(charid)
    ...
    TriggerClientEvent('loaf_character:loadCharacters', src, GetPlayerCharacters(src), true)
end)
...
function SetIdentifierToChar(identifier, charid)
    for _, itable in pairs(IdentifierTables) do
        -- MySQLAsyncExecute(('UPDATE %s SET %s = Char%s%s WHERE %s = %s'):format(itable.table, itable.column, charid, GetIdentifierWithoutSteam(identifier), itable.column, identifier))
...
function GetSpawnPos(source)
    ...
    if SpawnPos[1].position == nil then 
        return {x= 403.08, y = -996.81, z = -99.98}
    else
